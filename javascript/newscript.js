...
let ussschwartz = new Ussship('Ussschwartz', 20, 5, .7, "UssSchwartz")
...
while(battleBegin !== "yes" && battleBegin !== "no"){
...
if(battleBegin === "yes"){
    ...
    while(ussschwartz.hull > 0){
        while(attackOrRetreat !== "attack" && attackOrRetreat !== "retreat"){
        ...
        if(attackOrRetreat === "attack"){
            if(ussschwartz.attack(alienShips[activeEnemy])){
                activeEnemy++;
                attackOrRetreat = window.prompt("Enemy destroyed. Attack next ship or retreat?", "attack/retreat") 
            }else{
